...
const downloadFileHttps = (fileName, url, fallbackHash, integrity) => {
    let [hashType, expectedHash] = integrity?.split("-");
	if (hashType != undefined && expectedHash != undefined) {
		expectedHash = Buffer.from(expectedHash, "base64").toString("hex");
	} else {
		hashType = "sha1";
	...
	return new Promise((resolve, reject) => {
		https.get(url, (res) => {
			...
			res.on('end', () => {
				...
				const h = hash.read()
                if (h != expectedHash) {
                    return reject(new Error(`hash mismatch, expected ${expectedHash}, got ${h} in download for ${fileName}, ${url}`))
                }
                resolve()
			})
...
const downloadPkg = (pkg, verbose) => {
    console.log("downloading package:", pkg.resolved);
	const [ url, fallbackHash ] = pkg.resolved.split('#')
	...
	if (url.startsWith('https://codeload.github.com/') && url.includes('/tar.gz/')) {
		...
		return downloadGit(fileName, url.replace(/^git\+/, ''), fallbackHash)
	} else if (url.startsWith('https://')) {
		return downloadFileHttps(fileName, url, fallbackHash, pkg.integrity)
...
const prefetchYarnDeps = async (lockContents, verbose) => {
    ...
    console.log("lockfile data:", lockData);
